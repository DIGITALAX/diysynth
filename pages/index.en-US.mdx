...
<div className="max-w-xs mx-auto mt-5 mb-8 text-center text-offWhite install">
  ...
  >
    <Nextra.Tab>
      `The craft of image synthesis starts with well chosen words.`
    </Nextra.Tab>
    <Nextra.Tab>
      `Add the right modifiers to draw out the full potential of your prompts.`
    </Nextra.Tab>
    <Nextra.Tab>
      `There is a balance in all things, prompt tokens included.`
    </Nextra.Tab>
    <Nextra.Tab>
      `Personalize image generation with DIY datasets, models, and more.`
    </Nextra.Tab>
    <Nextra.Tab>`Not just for making music anymore.`</Nextra.Tab>
    <Nextra.Tab>`Performance metrics for optimal hardware config.`</Nextra.Tab>
  </Nextra.Tabs>
</div>
{/* prettier-ignore-end */}
